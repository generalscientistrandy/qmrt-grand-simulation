"""
3D Branch F v2: Spatial Coupling Gradient in Three Dimensions
=============================================================

Stage 2 of 3D validation.

Question: Does spatial coupling bias 3D topological populations
the way it did in 2D?

Test:
- Spherical coupling gradient (high center, low edge)
- Smooth cosine transition
- Track line density by region

Success criteria:
- Higher line density in high-coupling region
- Higher persistence there
- Regeneration bias toward high-coupling region
"""
...
class BranchF3DSimulator:
    """
    3D Branch F v2: Spatial coupling gradient for vortex-line control.
    """
    ...
    def __init__(
        ...
        # Spatial coupling field (spherical gradient)
    ...
    def _create_spherical_coupling(self) -> np.ndarray:
        ...
        # Interior radius (where coupling is highest)
...
def run_3d_spatial_coupling_test(size: int = 64, steps: int = 2500, sample_interval: int = 50):
    """
    Stage 2 test: Does spatial coupling bias 3D vortex-line populations?
    """
    print("="*70)
    print("3D BRANCH F v2: SPATIAL COUPLING TEST")
    print("="*70)
    print()
    print(f"Grid size: {size}³")
    print(f"Coupling: center={0.7}, edge={0.2}")
    print(f"Steps: {steps}")
    print()
    
    sim = BranchF3DSimulator(
        size=size, gamma=0.007,
        coupling_center=0.7, coupling_edge=0.2,
    ...
    print("Late-time vortex distribution:")
    print(f"  Interior:   {late_interior:.1f} ({100*late_interior/late_total:.1f}%)")
    print(f"  Transition: {late_transition:.1f} ({100*late_transition/late_total:.1f}%)")
    print(f"  Periphery:  {late_periphery:.1f} ({100*late_periphery/late_total:.1f}%)")
    ...
    print(f"  Interior:  {interior_density:.6f}")
    print(f"  Periphery: {periphery_density:.6f}")
    
    if interior_density > 0 and periphery_density > 0:
        density_ratio = interior_density / periphery_density
        print(f"  Ratio (interior/periphery): {density_ratio:.2f}×")
    print()
    ...
    interior_frac = late_interior / late_total if late_total > 0 else 0
    
    if interior_density > periphery_density * 1.5:
        print("✓ SUCCESS: Interior vortex density is significantly higher")
        print(f"  Interior density {density_ratio:.2f}× periphery density")
        success = True
    elif interior_density > periphery_density:
        print("~ PARTIAL: Interior density higher, but not strongly")
        success = False
    else:
        print("✗ FAILURE: No spatial bias toward high-coupling region")
        success = False
    ...
    return {
        ...
        'density_ratio': density_ratio if 'density_ratio' in dir() else 0,
        'time_series': {
...
if __name__ == "__main__":
    results = run_3d_spatial_coupling_test(size=64, steps=2500, sample_interval=50)
