...
EXCHANGE LOOP γ_ex:
  ...
  TOPOLOGY OF γ_ex:
    - In LABELED (ordered) configuration space: γ_ex is CONTRACTIBLE
      (the two particles just swap — no winding in M × M \ Δ)
    - In UNLABELED (quotient) configuration space: γ_ex is NON-CONTRACTIBLE
      (after quotienting by S₂, exchange becomes a nontrivial loop)
  
  This is the standard exchange-loop formalism:
    γ_ex is trivial upstairs, nontrivial downstairs.
...
ADMISSIBLE TRANSPORT CLASS (Explicit Definition):
  ...
  DEFINITION: The admissible transport class is the set of flat U(1) 
  connections on C that are:
    1. Generated by the Y-junction spinor transport rule
    2. Consistent with the spinor overlap formula ⟨e_out|e_in⟩
    3. Related by single-valued gauge transformations
    
  Formally:
    A_adm = { A : A is induced by Y-junction transport } / (gauge equivalence)
    
  This is the class within which we prove gauge non-removability.
  We do NOT claim results about connections outside this class.
...
Let C be the defect configuration space (2D, unordered, coincidence 
removed), and let A be the flat U(1) connection canonically determined
by the Y-junction transport rule.
...
THEN:
  ...
  2. This holonomy is INVARIANT under all single-valued gauge 
     transformations within the admissible transport class.
     
  3. Therefore, NO admissible gauge transformation trivializes 
     the exchange phase.
     
  4. ASSUMING physical states are sections of the associated line 
     bundle, the allowed state space lies in the SIGN REPRESENTATION 
     sector.
...
More precisely: a fermionic sector is SELECTED by geometry.
...
"QMRT yields a geometry-induced flat U(1) transport structure on the 
defect configuration space. The Y-junction transport rule CANONICALLY
SELECTS a distinguished flat connection within the admissible class.

For the exchange loop γ_ex (which is contractible in labeled space but 
non-contractible in the unlabeled quotient), this transport has 
nontrivial holonomy -1. This holonomy is gauge-invariant and not 
removable by any single-valued gauge transformation within the 
...
The admissible transport class is:
  { flat U(1) connections induced by Y-junction transport rule }
  modulo single-valued gauge equivalence.

This explicit definition removes all ambiguity.
...
2. SOFTENED UNIQUENESS
   - "Canonically selects" (not "uniquely determines")
   - "Distinguished" (not "the only")
   - No overclaim about full moduli space classification

3. ADMISSIBLE CLASS DEFINED
   - Explicit definition provided
   - Results scoped to this class only

4. ASSUMPTION VISIBLE
