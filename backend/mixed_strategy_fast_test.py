"""
Fast Mixed Strategy Sweep - 32³ Grid
=====================================

Reduced grid for fast falsification pass.
...
class FastMixedSimulator:
    ...
    def seed_balanced_structure(self, n_pairs: int = 10):
        ...
        for i in range(n_pairs):
            ...
            radius1 = np.random.uniform(2, self.size * 0.20)
            cx1 = int(np.clip(center + radius1 * np.cos(angle1), 4, self.size - 4))
            cy1 = int(np.clip(center + radius1 * np.sin(angle1), 4, self.size - 4))
            cz1 = center
            
            angle2 = angle1 + np.random.uniform(0.5, 1.5)
            radius2 = np.random.uniform(2, self.size * 0.20)
            cx2 = int(np.clip(center + radius2 * np.cos(angle2), 4, self.size - 4))
            cy2 = int(np.clip(center + radius2 * np.sin(angle2), 4, self.size - 4))
            cz2 = center
    ...
    def select_creation_location(self) -> Tuple[int, int, int]:
        ...
        radius = np.random.uniform(2, self.size * 0.20)
        cx = int(np.clip(center + radius * np.cos(angle), 4, self.size - 4))
        cy = int(np.clip(center + radius * np.sin(angle), 4, self.size - 4))
        return cx, cy, center
    
    def check_reoccupation(self, cx: int, cy: int, window: int = 80) -> bool:
        recent = [(s[0], s[1]) for s in self.creation_sites if self.step_count - s[3] < window]
        for sx, sy in recent:
            if np.sqrt((cx - sx)**2 + (cy - sy)**2) < 4:
                return True
        return False
    ...
    def attempt_spontaneous_creation(self):
        ...
        n_sample = min(30, n_candidates)
        indices = np.random.choice(n_candidates, n_sample, replace=False)
        
        for idx in indices:
            cx = candidates[0][idx]
            local_tau = self.tau[cx, candidates[1][idx], candidates[2][idx]]
            ...
            if np.random.random() < probability:
                ...
                if self.check_reoccupation(loc_cx, loc_cy):
                    self.reoccupation_count += 1
                
                offset_angle = np.random.uniform(0, 2 * np.pi)
                offset_dist = np.random.uniform(2, 4)
                cx2 = int(np.clip(loc_cx + offset_dist * np.cos(offset_angle), 2, self.size - 2))
                cy2 = int(np.clip(loc_cy + offset_dist * np.sin(offset_angle), 2, self.size - 2))
    ...
    def step(self, dt: float = 0.04):
        ...
        topology = gaussian_filter(topology, sigma=1.0)
    ...
    def compute_metrics(self) -> Dict:
        ...
        mean_nn = float(np.mean(np.min(dist_matrix, axis=1)))
        ...
        reocc = self.reoccupation_count / max(1, self.spontaneous_creations)
        
        return {'entropy': float(entropy), 'mean_nn_dist': mean_nn, 'reocc_rate': float(reocc)}


def run_test(rate: float, frac: float, steps: int = 600) -> Dict:
    sim = FastMixedSimulator(size=32, tau_creation_threshold=1.001,
                             creation_rate=rate, remnant_fraction=frac, remnant_alpha=1.0)
    
    np.random.seed(42)
    sim.psi_r += 0.03 * np.random.randn(32, 32, 32)
    sim.psi_i += 0.03 * np.random.randn(32, 32, 32)
    sim.seed_balanced_structure(n_pairs=10)
    ...
    for step in range(steps):
        sim.step()
        if step >= steps - 200 and step % 40 == 0:
    ...
    return {
        'rate': rate, 'frac': frac,
        'avg_n': avg_n, 'creations': sim.spontaneous_creations,
        'sustained': avg_n > 0.2,
        **metrics
    }
...
def main():
    print("=" * 85)
    print("  FAST MIXED STRATEGY SWEEP (32³)")
    print("=" * 85)
    print("\nQuestion: Can weak remnant bias beat pure random at rate ≤ 0.15?")
    print()
    ...
    names = {0.0: "Random", 0.10: "10% Mem", 0.25: "25% Mem", 
             0.50: "50% Mem", 0.75: "75% Mem", 1.0: "Pure Mem"}
    
    results = []
    
    print(f"{'Strategy':>10} │ {'Rate':>6} │ {'Avg N':>6} │ {'Creates':>7} │ {'Entropy':>7} │ {'NN Dist':>7} │ {'Reocc%':>6} │ {'OK':>3}")
    print("-" * 80)
    ...
    for frac in fracs:
        for rate in rates:
            ...
            print(f"{names[frac]:>10} │ {rate:>6.3f} │ {r['avg_n']:>6.2f} │ {r['creations']:>7} │ "
                  f"{r['entropy']:>7.3f} │ {r['mean_nn_dist']:>7.2f} │ {r['reocc_rate']*100:>5.1f}% │ {ok:>3}")
        print()
    ...
    for frac in fracs:
        ...
        if sus:
            ...
            print(f"  {names[frac]:>10}: {min_rate:.3f}")
        else:
            print(f"  {names[frac]:>10}: NONE")
    ...
    for frac in [0.10, 0.25, 0.50]:
        ...
        if frac_sus:
            ...
            if frac_min < random_min:
                wins.append((names[frac], frac_min))
    
    if wins:
        print(f"\n✓ MIXED STRATEGY BEATS RANDOM (baseline: {random_min:.3f})!")
        for name, rate in wins:
            print(f"  {name}: sustains at rate={rate:.3f}")
        print("\n→ Remnant coupling VIABLE with weak memory bias")
    else:
    ...
    # Save
    output = '/app/backend/qmrt_topology/papers/mixed_strategy_fast_results.json'
    clean = [{k: (bool(v) if isinstance(v, (np.bool_, bool)) else
                  float(v) if isinstance(v, (np.integer, np.floating)) else v)
             for k, v in r.items()} for r in results]
    
    with open(output, 'w') as f:
        json.dump({'test': 'mixed_strategy_fast', 'grid': 32, 'results': clean}, f, indent=2)
